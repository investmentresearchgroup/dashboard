...
stock_px_col_dict = {'Total Shares Traded': float}
stock_px_file = 'data/Stock_Prices.csv'
stock_price_df = pd.read_csv(stock_px_file, low_memory=False,
                             parse_dates=['Date'])

# Convert price to float
stock_price_df['Closing Price VWAP (GHS)'] = pd.to_numeric(
    stock_price_df['Closing Price VWAP (GHS)'], errors='coerce')

stock_price_tickers = [{'label': i, 'value': i}
                       for i in stock_price_df['Share Code'].unique().tolist()]
...
def share_charac_func(share_code):
    ticker_url = 'https://dev.kwayisi.org/apis/gse/equities/{}'.format(
        share_code)
...
company_names = [stock_charact_list[i]['company']['name']
                 for i in range(39) if i != 31]

# Extract company share codes
company_share_codes = [stock_charact_list[i]['name']
                       for i in range(39) if i != 31]

# extract industry names
industry_names = [stock_charact_list[i]['company']['sector']
                  for i in range(39) if i != 31]

# Create DataFrame for extracted information
capital_company = pd.DataFrame(zip(company_names, industry_names, market_cap), columns=[
                               'Name', 'Sector', 'Capitalization'])
capital_industry = capital_company.groupby(
    by='Sector', as_index=False).Capitalization.sum().sort_values(by='Capitalization')

# Create dictionary of industry dfs
industry_share = [capital_company.loc[capital_company['Sector'] == sector]
                  for sector in industry_names[1:]]
...
def stock_share_func(df, ticker):
    ...
    stock_inception_price = stock_df.loc[(stock_df['Date'] == stock_inception_date),
                                         'Closing Price VWAP (GHS)'].values[0]
    stock_inception_shares = 10000 / stock_inception_price
    return stock_inception_shares


# Assign function to a variable
start_shares_list = [stock_share_func(
...
def annual_rate_func(df, stock):
    ...
    begin = list(df.loc[df['Share Code'] == stock].groupby(by='Year', as_index=False).apply(
        lambda x: x[x['Date'] == x['Date'].min()]['Closing Price VWAP (GHS)'].mean()))
    end = list(df.loc[df['Share Code'] == stock].groupby(by='Year', as_index=False).apply(
        lambda x: x[x['Date'] == x['Date'].max()]['Closing Price VWAP (GHS)'].mean()))
    year = stock_price_df.loc[stock_price_df['Share Code']
                              == stock]['Year'].unique().tolist()
    annual_df = pd.DataFrame(list(zip(year, end, begin)), columns=[
                             'Year', 'Close', 'Start'])
    annual_df['Returns'] = annual_df.apply(lambda x: (
        x['Close'] - x['Start']) / x['Start'], axis=1)
    return annual_df


# Create a list of the annual returns df calculated based on the annual return function
annual_return_list = [annual_rate_func(
    stock_price_df, share) for share in share_code_list]
                            ...
                            dbc.Card(
                                dbc.CardBody(
                                    [
                                        ...
                                        html.Div(
                                            "The company focuses on mining activities")
                                    ]
                                )
                            )
                        )
